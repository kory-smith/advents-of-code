...
function getVertices(inputWithNewlines: string) {
  const rowLength = inputWithNewlines.split("\n")[1].length;
  const inputWithoutNewlines = inputWithNewlines.replace(/\n/g, "");
  const startingPoint = inputWithoutNewlines.indexOf("S");
  const startShape = determineSTrueIdentity(inputWithNewlines);
  ...
  // How can we turn a one-dimensional array back into a two-dimensional one?
  // . . (0)
  // . . (1)
  // . . (2)
  // Becomes . . . . . .
  // 	                 ^
  // Let's say we're here. We're at index 5. Our row length is 2. What can we learn?
  ...
  do {
    ...
    const x = (currentPoint % rowLength) + 1;
    const y = Math.floor(currentPoint / rowLength);
...
/* 
	Last time, I calcualted the circumference of the shape. Now, I calcualte its area?
	I cheated: Reddit said I can use Pick's theorem.
	A = i + b/2 - 1.
	i = A - (b / 2) + 1
	b is the circumference. A is area. I'm solving for i.
	Which means I need to find A.
 */

type Vertices = Array<[number, number]>;
// AI wrote this. Credit where it is due.
function calculateArea(vertices: Vertices) {
  let area = 0;
...
console.log(
  calculateArea(getVertices(input)) - walkPipesAndReportLength(input) / 2 + 1
);
