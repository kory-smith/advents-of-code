...
function stringToDigit(string) {
  ...
  let nextDigitToReplace = doodad(string);

  while (nextDigitToReplace !== null) {
    const stringDigitRegex = new RegExp(nextDigitToReplace, "g");
    finalString = finalString.replace(
      stringDigitRegex,
      map[nextDigitToReplace]
    );
    nextDigitToReplace = doodad(finalString);
  }
  return finalString;
}

const fucker = "zoneight234";

// handle strings like "eightwothree"
function doodad(string) {
  let helperIndex = Infinity;
  let numberDigitToReturn = null;

  for (const stringDigit of Object.keys(map)) {
    const index = string.indexOf(stringDigit);
    if (index >= 0 && index < helperIndex) {
      numberDigitToReturn = stringDigit;
      helperIndex = index;
    }
  }
