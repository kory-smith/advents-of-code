...
function calculateTotalGearRatio(schematic) {
  ...
  const gearIndicesThatTouchNumbers = {};
  ...
  while ((match = digitsRE.exec(flatSchematic)) !== null) {
    let touchingGearIndex = check({
      endIndex: match.index + match[0].length,
      startIndex: match.index,
      rowLength: rowLength,
      schematic: flatSchematic,
    });
    if (touchingGearIndex) {
      const record = gearIndicesThatTouchNumbers[touchingGearIndex];
      if (record) {
        gearIndicesThatTouchNumbers[touchingGearIndex].push(match[0]);
      } else gearIndicesThatTouchNumbers[touchingGearIndex] = [match[0]];
    }
  }

  let total = 0;
  for (const relevantGearIndex in gearIndicesThatTouchNumbers) {
    const current = gearIndicesThatTouchNumbers[relevantGearIndex];

    if (current.length === 2) {
      const first = Number(current[0]);
      const second = Number(current[1]);
      total += first * second;
    }
  }
  return total;
...
function check({ schematic, rowLength, startIndex, endIndex, partNumber }) {
  ...
  for (const index of indicesToCheck) {
    // Right
    if (gear.exec(schematic[index + 1])) return index + 1;
    // Left
    else if (gear.exec(schematic[index - 1])) return index - 1;
    // Below
    else if (gear.exec(schematic[index + rowLength])) return index + rowLength;
    // Below right
    else if (gear.exec(schematic[index + rowLength + 1]))
...
console.log(calculateTotalGearRatio(input));
