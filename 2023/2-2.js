...
const testInputAnswer = 2286;

const colors = ["red", "blue", "green"];

function determinePowerSums(gameList) {
  const powers = [];
  const splitInput = gameList.split("\n");

  for (const game of splitInput) {
    const gameWithoutHeader = game.split(":")[1];
    const minimums = [];
    colors.forEach((color) => {
      minimums.push(getMinimumCubesForColor(color, gameWithoutHeader));
    });
    powers.push(minimums.reduce((a, b) => a * b));
		console.log(`Minimums for ${game}: ${minimums}`)
  }
...
function getMinimumCubesForColor(color, game) {
  const roundMinimums = [];

  for (const round of game.split(";")) {
    const regex = new RegExp(`(\\d+) ${color}`, "g");

    const matches = round.match(regex);

		if (!matches) {
			continue;
		}

    const justNumbers = matches.map((thing) => parseInt(thing.split(" ")[0]));

    justNumbers.forEach((number) => roundMinimums.push(number));
  }

	// I had initially used Math.min here, but we need the MAX per round
  return Math.max(...roundMinimums);
}

console.log(determinePowerSums(input));
