...
function determinePossibleGames(gameList) {
  const validGames = [];
  const splitInput = gameList.split("\n");

  splitInput.forEach((game) => {
    validGames.push(isGameValid(game));
  });

  return validGames.filter(val => val).map(val => parseInt(val)).reduce((a, b) => a + b);
}

// I first wrote this mistakenly thinking I should get only INVALID games
function isGameValid(gameString) {
  const gameId = gameString.split(":")[0].replace(/\D/g, "");
  const games = gameString.split(":")[1].split(";");

  // 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
  for (const game of games) {
    for (const color of colors) {
      const relevantMax = maxes[color];
      if (calculateGameMaxForColor(color, game) > relevantMax) {
				console.log(`Game ${gameId} is bad because of color ${color}`)
				// Game is impossible
        return 0;
      }
    }
...
function calculateGameMaxForColor(color, game) {
  let max = -Infinity;
  const rounds = game.split(";");
  for (const round of rounds) {
    const colorScore = calculateRoundForColor(color, round);
    if (colorScore > max) max = colorScore;
  }
  return max;
}

// round: 3 green, 4 blue, 1 red
function calculateRoundForColor(color, round) {
...
console.log(determinePossibleGames(input));
